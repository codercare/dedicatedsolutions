...
if( ! class_exists('acf_svg_select') ) :

class acf_svg_select extends acf_field {
	...
	*/
	...
	function initialize() {
		...
		$svg_files = preg_grep('~\.(svg)$~', scandir(DS_THEME_PATH . "assets/svg-icons"));
		$default_choices = array();
		foreach ( $svg_files as $svg) {
			$choice_key = strtolower( str_replace(" ", "_", $svg) );
			$choice_key = preg_replace('/\\.[^.\\s]{3,4}$/', '', $choice_key);
			$default_choices[$choice_key] = $svg;
		} 
		
		// vars
		$this->name = 'svg_select';
		$this->label = _x('Select SVG', 'noun', 'acf');
	...
	*/
	...
	function input_admin_enqueue_scripts() {
		...
		// scripts
		wp_register_script('acf-input-svg-select', get_theme_file_uri( '/assets/js/acf-svg-select.js' ), array(), wp_get_theme()->get( 'Version' ), true);
		wp_enqueue_script('acf-input-svg-select');
	...
	*/
	...
	function render_field( $field ) {
		...
		$select = array(
			'id'				=> $field['id'],
			'class'				=> $field['class'] . 'acf-svg-select',
			'name'				=> $field['name'],
			'data-ui'			=> $field['ui'],
			...
			'data-image_path'	=> get_template_directory_uri() . '/assets/svg-icons/'
		);
		...
		// render
		$value = (array) acf_extract_var( $select, 'value' );
		$choices = (array) acf_extract_var( $select, 'choices' );
		echo sprintf(
			'<select %s>%s</select>',
			acf_esc_attrs($select),
			$this->custom_acf_walk_select_input($choices, $value)
		);
	...
	function custom_acf_walk_select_input( $choices = array(), $values = array(), $depth = 0 ) {
		$html = '';
...
// initialize
acf_register_field_type( 'acf_svg_select' );
